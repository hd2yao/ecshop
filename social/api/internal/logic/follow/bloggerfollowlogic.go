...
func (l *BloggerFollowLogic) BloggerFollow(req *types.FollowRequest) (resp *types.Response, err error) {
	...
	// 3. 调用 RPC
	rpcResp, err := l.svcCtx.SocialRpc.BloggerFollow(l.ctx, rpcReq)
	if err != nil {
		l.Errorf("调用 BloggerFollow RPC 失败: %v", err)
		return &types.Response{
