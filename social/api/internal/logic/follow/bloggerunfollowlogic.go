...
// NewBloggerUnfollowLogic 博主取消关注
...
func (l *BloggerUnfollowLogic) BloggerUnfollow(req *types.FollowRequest) (resp *types.Response, err error) {
	...
	// 3. 调用 RPC
	rpcResp, err := l.svcCtx.SocialRpc.BloggerUnfollow(l.ctx, rpcReq)
	if err != nil {
		l.Errorf("调用 BloggerUnfollow RPC 失败: %v", err)
		return &types.Response{
