...
// NewFollowerUnfollowLogic 粉丝取关（取消对粉丝的关注）
...
func (l *FollowerUnfollowLogic) FollowerUnfollow(req *types.FollowRequest) (resp *types.Response, err error) {
	...
	// 3. 调用 RPC
	rpcResp, err := l.svcCtx.SocialRpc.FollowerUnfollow(l.ctx, rpcReq)
	if err != nil {
		l.Errorf("调用 FollowerUnfollow RPC 失败: %v", err)
		return &types.Response{
